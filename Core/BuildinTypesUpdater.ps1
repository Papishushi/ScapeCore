...
Get-ChildItem -Recurse -Filter '*.cs' | ForEach-Object {
    ...
    $matchedTypes = [Regex]::Matches($content, '(?<=public class\s|public interface\s|public virtual\s|public abstract class\s|public sealed class\s|public record struct\s|public readonly struct\s|public readonly record struct\s|public readonly record class\s|public readonly class\s|public record class\s|public enum\s|public struct\s)\w+')

    foreach ($match in $matchedTypes) {
